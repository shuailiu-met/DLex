...
class NeuralNetwork():
    ...
    def forward(self):
        ...
        for each_layer in self.layers:  # run over all layers ,fullyconnected layer,the last one is activation layer
            ...
            regularization_loss += each_layer.optimizer.regularizer.norm(each_layer.weight)

        if self.loss_layer is not None:
            loss_total = self.loss_layer.forward(input_tensor, label_tensor) + regularization_loss # compute cross entropy loss by the output of activation layer
            return loss_total
    ...
    def append_trainable_layer(self, layer):  # assign optimizer to each layer and apply initializer on each layer
    ...
    @phase.setter
    def phase(self, _phase_setting):
        self._phase = _phase_setting

    def train(self, iterations):
        #iterations is a int number
        self.phase = False
