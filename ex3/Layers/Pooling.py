...
class Pooling(Base.BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        out_y = int((y - y % pooling_y) / self.stride_shape[
            0])  # remove the part which is not large enough for one more pooling
        out_x = int((x - x % pooling_x) / self.stride_shape[
            1])  # remove the part which is not large enough for one more pooling
            ...
            for channel in range(c):
                for out_y_idx in range(out_y):
                    for out_x_idx in range(out_x):
                        ...
                        temp_array = input_tensor[batch, channel, pool_origin_y:(pool_origin_y + pooling_y),
                                     pool_origin_x:(pool_origin_x + pooling_x)]
                        max_value = np.max(temp_array)
                        output[batch, channel, out_y_idx, out_x_idx] = max_value

                        temp_coord_y, temp_coord_x = np.where(
                            temp_array == max_value)  # take the 0 index(there might be some idx with same value)
            ...
            for channel in range(c):
                for out_y_idx in range(out_y):
                    for out_x_idx in range(out_x):
                        ...
                        output_error[
                            batch, channel, co_y, co_x] += value  # some overlaping points have done contribution to 2 lapping,so the error tensor value is summed for this point
