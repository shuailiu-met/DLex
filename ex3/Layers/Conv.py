...
class Conv(BaseLayer):
    ...
    @optimizer.setter
    def optimizer(self,optimizer_obj):#_optimizer is a object of type sgd
        self._optimizer = copy.deepcopy(optimizer_obj)
        self._optimizer_forbias = copy.deepcopy(optimizer_obj)
    ...
    def backward(self,error_tensor):
        ...
        temp_error = np.zeros((np.shape(self.input_tensor)[0],np.shape(self.input_tensor)[1],self.num_kernels,*np.shape(self.input_tensor)[2:]))   # b c k y/ b c k y x
        for batch in range(b):
            for channel in range(c):
                for kernel in range(self.num_kernels):
        ...
        if (len(self.input_tensor.shape) == 4): #3d
            ...
            for batch in range(b):
                for channel in range(c):
                    pad_input[batch,channel,int(n/2):(y + int(n/2))] = self.input_tensor[batch,channel,:]#the first m/2 elemnts and the last m/2 will keep 0
        ...
        temp_result = np.zeros((b,k))
        for batch in range(b):
            for kernel in range(k):
                temp_result[batch,kernel] = np.sum(error_tensor[batch,kernel,:])# sum of elements error
        self.gradient_bias = np.sum(temp_result,axis=0)#sum along batch dim
