...
class FullyConnected(Base.BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        new_input = np.concatenate((input_tensor, bias),
                                   axis=1)  # add homogeneous part,inputsize as columns,so add a column of 1 on the right end
    ...
    def backward(self, error_tensor):
        ...
        temp_gradient_weights = np.dot(np.transpose(self.input_tensor),
                                       error_tensor)  # need to store input tensor in forward step,input tensor is always with homogenous dim
        self.gradient_weights = temp_gradient_weights  # setter used
        # update weights
        if self.optimizer is not None:
            self.weights = self._optimizer.calculate_update(self.weights,
                                                            self.gradient_weights)  # update weights and errors,new weights is updated for the forward,so bias need
        return gradient_x
