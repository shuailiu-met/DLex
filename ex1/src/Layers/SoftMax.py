...
class SoftMax():
    ...
    def backward(self,error_tensor):#here we need yhat, product in backward step is scalar,so we finally get a (9,4)array
        #print(np.shape(error_tensor))#(9*4)
        #yhat(9*4) scalar product
        scalar_product = error_tensor*self.probability_tensor
        sum_s_p = np.expand_dims(np.sum(scalar_product,axis = 1),axis = 1) #(9x1)array
        En_1 = self.probability_tensor*(error_tensor-sum_s_p)#(9,4)array - (9,1)array means all elements of each row - corresponding number
        return En_1
